...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        ...
        success_rate = spacex_df.groupby('Launch Site')['class'].mean().reset_index()
        fig = px.pie(success_rate, names='Launch Site', values='class', title='Success Rate for All Sites')
    else:
...
def update_scatter_chart(entered_site, payload_range):
    if entered_site == 'ALL':
        ...
        filtered_df = spacex_df[(spacex_df['Launch Site'] == entered_site) & 
                                ...
                                (spacex_df['Payload Mass (kg)'] <= payload_range[1])]
        fig = px.scatter(filtered_df, x='Payload Mass (kg)', y='class', color='Booster Version Category',
                         title=f'Correlation between Payload and Launch Outcome for {entered_site}')
    return fig
